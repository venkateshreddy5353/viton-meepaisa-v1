...
# model image list for web server
...
cloth_list_raw = os.listdir(os.path.join(BASE_DIR, "static/web_option", "cloth"))
cloth_list = []
c_counter = 0
...
@app.route('/web')
def hello_world():
    return render_template('login.html', person_list=person_list, img_list=cloth_list)
...
def upload_image():
    ...
    if (not len(request.files) == 2 or (len(request.form) == 1 and len(request.files) == 1)):
        return render_template('login.html', info="selection error", person_list=person_list, img_list=cloth_list)
    else:
        c_index = 0  # init
        p_index = 0 # init
        cloth_image = None
        ...
        # 4 combinations of uploading and selecting model or cloth images 
        if 'p_optionsRadios' in request.form:
            p_index = int(request.form['p_optionsRadios'][6:])
            person_image = person_list[p_index][0]
            if 'c_optionsRadios' in request.form:
                c_index = int(request.form['c_optionsRadios'][6:])
            else:
                cloth_image = request.files['cloth_image']
        
        if 'p_optionsRadios' not in request.form:
        ...
        if o_name is None: # bad cloth image
            ...
            return render_template(
                ...
                result1=h_name,
                result2=o_name,
                info="time: %.3f" % (end_time-start_time))
...
def run_model_web(f, cloth_name, cloth_f=None):
    ...
    if cloth_f is None:
        print(f, cloth_name)
        c_img = np.array(Image.open(os.path.join("static/web_option","cloth",cloth_name)))
    else:
        print(f, cloth_f)
        try:
